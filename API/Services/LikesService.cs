...
public class LikesService : ILikesService
{
    ...
    public async Task<bool> ToggleLikeAsync(int sourceUserId, int targetUserId)
    {
        ...
        var existingLike = await _unitOfWork.LikesRepository.GetUserLike(
            sourceUserId,
            targetUserId
        );
        ...
        if (existingLike == null)
        {
            ...
            _logger.Information(
                "User {SourceUserId} liked User {TargetUserId}",
                sourceUserId,
                targetUserId
            );
        }
        else
        {
            ...
            _logger.Information(
                "User {SourceUserId} unliked User {TargetUserId}",
                sourceUserId,
                targetUserId
            );
        }
        ...
        {
            _logger.Error(
                "Failed to update like status between SourceUserId: {SourceUserId} and TargetUserId: {TargetUserId}",
                sourceUserId,
                targetUserId
            );
        }
