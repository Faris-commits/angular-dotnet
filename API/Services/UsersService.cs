    ...
    public class UsersService : IUsersService
    {
        ...
        public async Task<PhotoDto> AddPhotoAsync(string username, IFormFile file)
        {
            ...
            if (result.Error != null)
                throw new InvalidOperationException("Error uploading photo to Cloudinary");
            ...
            _logger.LogInformation(
                "Photo uploaded successfully for user {Username} with publicId {PublicId}",
                username,
                result.PublicId
            );
        ...
        public async Task<bool> DeletePhotoAsync(string username, int photoId)
        {
            ...
            if (photo.IsMain)
                throw new InvalidOperationException("You can't delete your main photo");

            if (photo.PublicId != null)
            {
                var result = await _photoService.DeletePhotoAsync(photo.PublicId);
                if (result.Error != null)
                    throw new InvalidOperationException("Error deleting photo from Cloudinary");
            }
            ...
            _logger.LogInformation(
                "Photo with ID {PhotoId} deleted successfully for user {Username}",
                photoId,
                username
            );
        ...
        public async Task<PagedList<MemberDto>> GetUsersAsync(
        ...
        public async Task SetPhotoTagsAsync(string username, int photoId, List<int> tagIds)
        {
            ...
            if (duplicateTags.Any())
                throw new InvalidOperationException(
                    "Duplicate tags detected. Tags must be unique."
                );
        ...
        public async Task UpdateUserAsync(string username, MemberUpdateDto memberUpdateDto)
        {
            ...
            if (memberUpdateDto == null)
                throw new ArgumentNullException(
                    nameof(memberUpdateDto),
                    "Member update data cannot be null."
                );

            _logger.LogDebug(
                "UsersService - UpdateUserAsync invoked (username: {Username})",
                username
            );
        ...
        public async Task<bool> DeleteTagAsync(int tagId)
        {
            ...
            {
                _logger.LogInformation(
                    "Removing {Count} photo tags associated with tag ID {TagId}.",
                    photoTags.Count,
                    tagId
                );
